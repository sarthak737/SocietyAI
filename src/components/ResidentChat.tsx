  ...
  useEffect(() => {
    ...
    evtSource.onmessage = (event) => {
      try {
        ...
        // Filter for current user: messages where senderId or receiverId matches
        const userMessages = allMessages.filter(
          (msg: Message) => msg.senderId === userId || msg.receiverId === userId
        )
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/resident/messages', {
        ...
        body: JSON.stringify({ content: input.trim(), complaintId })
      })
      setInput('')
    } catch (e) {
          ...
          <div
            ...
            className={`max-w-[80%] p-2 rounded-lg text-sm ${
              msg.isFromAdmin
                ...
                : 'bg-white dark:bg-slate-700 text-gray-800 dark:text-gray-100 rounded-tr-none ml-auto'
            }`}
          >
            {msg.content}
          </div>
